...
Under the hood, this component is made up of a `HOC`(Higher Order Component) which is also expoed should you require more "fine-tuned" control over its usage.  An example using with a library such as [recompose](https://github.com/acdlite/recompose) is as follows:

```js
import { withCrop } from 'react-image-crop'

const myWrappedComponent = compose(
  withCrop,For a full example, take a look at the how [ReactCrop.js](./lib/ReactCrop.js) is wrapped from withCrop
  withOtherHOC,
  withAnotherHOC
)(MyComponent)
```
