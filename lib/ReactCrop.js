...
class ReactCrop extends PureComponent {
  ...
  onComponentMouseTouchDown = (e) => {
    const { disabled, keepSelection, onChange } = this.props;

    if (e.target !== this.imageRef) {
      return;
    }

    if (disabled || keepSelection) {
      return;
    }
  ...
  onImageLoad = (e) => {
    this.props.onImageLoaded(e.target);
  }

  getPixelCrop(crop) {
    const { imageRef } = this;
    return {
      x: Math.round(imageRef.width * (crop.x / 100)),
      y: Math.round(imageRef.height * (crop.y / 100)),
      width: Math.round(imageRef.width * (crop.width / 100)),
      height: Math.round(imageRef.height * (crop.height / 100)),
    };
  }
  ...
  getNewSize() {
    const { newCropIsBeingDrawn } = this.state;
    ...
    if (!newCropIsBeingDrawn) {
      adjustedMaxWidth = (['nw', 'w', 'sw'].indexOf(evData.inversedXOrd || evData.ord) > -1) ?
        evData.cropStartX :
        100 - evData.cropStartX;
      adjustedMaxWidth = clamp(adjustedMaxWidth, 100, maxWidth);
    }

    newWidth = clamp(newWidth, minWidth, adjustedMaxWidth);
    ...
    // Stop the box expanding on the opposite side when some edges are hit.
    if (!newCropIsBeingDrawn) {
      adjustedMaxHeight = (['nw', 'n', 'ne'].indexOf(evData.inversedYOrd || evData.ord) > -1) ?
        evData.cropStartY :
        100 - evData.cropStartY;
      adjustedMaxHeight = clamp(adjustedMaxHeight, 100, maxHeight);
    }

    newHeight = clamp(newHeight, minHeight, adjustedMaxHeight);
  ...
  resizeCrop() {
    ...
    if (nextCrop.aspect || ReactCrop.xyOrds.indexOf(ord) > -1) {
      nextCrop.x = clamp(newX, 0, 100 - newSize.width);
      nextCrop.y = clamp(newY, 0, 100 - newSize.height);
