...
function makeAspectCrop(crop, imageWidth, imageHeight) {
  ...
  const isPixels = completeCrop.unit === 'px';
...
class ReactCrop extends PureComponent {
  ...
  getLimits() {
    ...
    // Convert to percent (min/max is in pixels since v7).
    return {
      minWidth: minWidth ? minWidth / this.imageRef.width * 100 : 0,
      maxWidth: maxWidth ? maxWidth / this.imageRef.width * 100 : 100,
      minHeight: minHeight ? minHeight / this.imageRef.height * 100 : 0,
      maxHeight: maxHeight ? maxHeight / this.imageRef.height * 100 : 100,
    };
  }
  ...
  getNewSize() {
    ...
    if (crop.aspect) {
      ...
      newHeight = isPixels ? evData.cropStartHeight + evData.yDiff : evData.cropStartHeight + evData.yDiffPc;
    }
    ...
    // If % we need to scale the new height we derived from the % width in order to maintain
    // the correct aspect.
    if (crop.aspect && !isPixels) {
      newHeight *= imageWidth / imageHeight;
    }
