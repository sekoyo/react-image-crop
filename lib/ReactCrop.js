...
class ReactCrop extends PureComponent {
  ...
  onComponentMouseTouchDown = (e) => {
    const {
      crop, disabled, keepSelection, onChange, delta,
    } = this.props;
    ...
    const xPc =
      (clientPos.x - imageOffset.left) / (this.imageRef.width * 100) / delta;
    const yPc =
      (clientPos.y - imageOffset.top) / (this.imageRef.height * 100) / delta;
  ...
  onDocMouseTouchMove = (e) => {
    ...
    const xDiffPx = (clientPos.x - evData.clientStartX) / delta;
    evData.xDiffPc = xDiffPx / (this.imageRef.width * 100);

    const yDiffPx = (clientPos.y - evData.clientStartY) / delta;
