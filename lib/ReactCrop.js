...
class ReactCrop extends PureComponent {
  ...
  createNewCrop(optionalCrop) {
    const { width, height } = this.getMediaDimensions();
    const crop = this.makeNewCrop(optionalCrop?.unit || 'px', optionalCrop);
    const resolvedCrop = resolveCrop(crop, width, height);
    const pixelCrop = convertToPixelCrop(resolvedCrop, width, height);
    const percentCrop = convertToPercentCrop(resolvedCrop, width, height);
  ...
  onLoad = optionalCrop => {
    ...
    const { pixelCrop, percentCrop } = this.createNewCrop(optionalCrop);
    onChange(pixelCrop, percentCrop);
    onComplete(pixelCrop, percentCrop);
  };
  ...
  makeNewCrop(unit = 'px', optionalCrop) {
    const crop = { ...ReactCrop.defaultCrop, ...(optionalCrop || this.props.crop) };
    const { width, height } = this.getMediaDimensions();
