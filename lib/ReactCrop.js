...
class ReactCrop extends PureComponent {
  ...
  onImageLoad(image) {
    const resolvedCrop = resolveCrop(this.props.crop, image);
    const pixelCrop = getPixelCrop(image, resolvedCrop);

    this.props.onImageLoaded(image, pixelCrop);
  ...
  resizeCrop() {
    ...
    if (!this.props.disableCrossover
        || (this.props.minWidth <= 0 && this.props.minHeight <= 0)) {
      this.crossOverCheck();
    }
...
ReactCrop.propTypes = {
  ...
  disableCrossover: PropTypes.bool,
