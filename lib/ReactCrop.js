...
function resolveCrop(crop, image) {
  ...
  if (crop.unit === 'pc') {
    resolvedCrop = convertToPixelCrop(crop, image.width, image.height);
  }

  if (resolvedCrop && resolvedCrop.aspect && isAspectInvalid(resolvedCrop, image)) {
    return makeAspectCrop(resolvedCrop, image.width, image.height);
  }
...
function containCrop(prevCrop, crop, imageWidth, imageHeight) {
  ...
  if (crop.aspect && prevPixelCrop.x > crop.x && (crop.height + crop.y) >= imageHeight) {
    contained.x = prevPixelCrop.x;
  }

  // Don't let the crop grow on the opposite side when hitting an x image boundary.
  let cropXAdjusted = false;
  if (contained.x + contained.width > imageWidth) {
    contained.width = crop.width + (imageWidth - (crop.x + crop.width));
    contained.x = crop.x + (imageWidth - (crop.x + contained.width));
    cropXAdjusted = true;
  ...
  if (cropXAdjusted && crop.aspect) {
    ...
    if (prevPixelCrop.y > contained.y) {
  ...
  if (contained.y + contained.height > imageHeight) {
    contained.height = crop.height + (imageHeight - (crop.y + crop.height));
    contained.y = crop.y + (imageHeight - (crop.y + contained.height));
    cropYAdjusted = true;
...
class ReactCrop extends PureComponent {
  ...
  onCropMouseTouchDown = (e) => {
    ...
    let cropOffset;
    const crop = convertToPixelCrop(this.props.crop, this.imageRef.width, this.imageRef.width);
