...
function getPixelCrop(image, percentCrop, useNaturalImageDimensions) {
  if (!image || !percentCrop) {
    return null;
  }

  const imageWidth = useNaturalImageDimensions ? image.naturalWidth : image.width;
  const imageHeight = useNaturalImageDimensions ? image.naturalHeight : image.height;
...
class ReactCrop extends PureComponent {
  ...
  onComponentMouseTouchDown = (e) => {
    ...
    this.mouseDownOnCrop = true;
    onChange(nextCrop, getPixelCrop(this.imageRef, nextCrop, useNaturalImageDimensions));
    this.setState({ cropIsActive: true });
  }
  ...
  onComponentKeyDown = (e) => {
    ...
    if (nudged) {
      ...
      onChange(nextCrop, getPixelCrop(this.imageRef, nextCrop, useNaturalImageDimensions));
      onComplete(nextCrop, getPixelCrop(this.imageRef, nextCrop, useNaturalImageDimensions));
    }
  }
  ...
  onDocMouseTouchEnd = () => {
    ...
    if (this.mouseDownOnCrop) {
      ...
      onDragEnd();
      onComplete(crop, getPixelCrop(this.imageRef, crop, useNaturalImageDimensions));
