...
function withCrop (BaseComponent) {

  class ReactCropHoc extends Component {
    static propTypes = {
    ...
    constructor(props) {
      ...
      BaseComponent.onDocMouseTouchMove = this.onDocMouseTouchMove.bind(BaseComponent)
      this.onDocMouseTouchEnd = this.onDocMouseTouchEnd.bind(this);
      BaseComponent.onDocMouseTouchEnd = this.onDocMouseTouchEnd.bind(BaseComponent)
      this.onImageLoad = this.onImageLoad.bind(this);
      BaseComponent.onImageLoad = this.onImageLoad.bind(BaseComponent)
      this.onComponentMouseTouchDown = this.onComponentMouseTouchDown.bind(this);
      BaseComponent.onComponentMouseTouchDown = this.onComponentMouseTouchDown.bind(BaseComponent)
      this.onComponentKeyDown = this.onComponentKeyDown.bind(this);
      BaseComponent.onComponentKeyDown = this.onComponentKeyDown.bind(BaseComponent)
      this.onCropMouseTouchDown = this.onCropMouseTouchDown.bind(this);
      BaseComponent.onCropMouseTouchDown = this.onCropMouseTouchDown.bind(BaseComponent)

      const nextCrop = this.nextCropState(props.crop, false);

      this.state = {
        crop: nextCrop,
        cropInvalid: isCropInvalid(nextCrop),
      };
    }
    ...
    onComponentKeyDown(e) {
      ...
      if (keyCode === ReactCropHoc.arrowKey.left) {
        crop.x -= ReactCropHoc.nudgeStep;
        nudged = true;
      } else if (keyCode === ReactCropHoc.arrowKey.right) {
        crop.x += ReactCropHoc.nudgeStep;
        nudged = true;
      } else if (keyCode === ReactCropHoc.arrowKey.up) {
        crop.y -= ReactCropHoc.nudgeStep;
        nudged = true;
      } else if (keyCode === ReactCropHoc.arrowKey.down) {
        crop.y += ReactCropHoc.nudgeStep;
        nudged = true;
      }
    ...
    onDocMouseTouchEnd() {
      ...
      if (this.mouseDownOnCrop) {
        ...
        this.setState({
          cropInvalid: isCropInvalid(crop),
    ...
    resizeCrop() {
      ...
      if (crop.aspect || ReactCropHoc.xyOrds.indexOf(ord) > -1) {
      ...
      } else if (ReactCropHoc.xOrds.indexOf(ord) > -1) {
        crop.x = clamp(newX, 0, 100 - newSize.width);
        crop.width = newSize.width;
      } else if (ReactCropHoc.yOrds.indexOf(ord) > -1) {
        crop.y = clamp(newY, 0, 100 - newSize.height);
        crop.height = newSize.height;
      }
    ...
    nextCropState(crop, validateCrop) {
      const nextCrop = { ...ReactCropHoc.defaultCrop, ...crop };
      if (validateCrop) {
        this.setState({ cropInvalid: isCropInvalid(nextCrop) })
      }
      return nextCrop;
    }
    ...
    render() {
      return <BaseComponent
        createCropSelection={this.createCropSelection.bind(this)}
        nextCropState={this.nextCropState.bind(this)}
        crossOverCheck={this.crossOverCheck.bind(this)}
        onDocMouseTouchMove={this.onDocMouseTouchMove.bind(this)}
        onImageLoad={this.onImageLoad.bind(this)}
        onComponentMouseTouchDown={this.onComponentMouseTouchDown.bind(this)}
        onComponentKeyDown={this.onComponentKeyDown.bind(this)}
        onCropMouseTouchDown={this.onCropMouseTouchDown.bind(this)}
        imageRef={(imageElement) => this.imageRef = imageElement}
        componentRef={(componentElement) => this.componentRef = componentElement}
        cropWrapperRef={(cropWrapperElement) => this.cropWrapperRef = cropWrapperElement}
        imageCopyRef={(imageCopyElement) => this.imageCopyRef = imageCopyElement}
